...
このドキュメントは、ExcelとGoogle Sheetsで利用可能なすべての関数を網羅的にまとめたものです。各関数の実装状況を示すマークを付けています。
...
| 関数名 | 説明 | 構文 | 実装状況 |
|--------|------|----------|
| BETADIST | ベータ分布（旧版） | `=BETADIST(x, α, β, [A], [B])` | ✅ |
| BETAINV | ベータ分布の逆関数（旧版） | `=BETAINV(確率, α, β, [A], [B])` | ✅ |
| BINOMDIST | 二項分布（旧版） | `=BINOMDIST(成功数, 試行回数, 成功率, 累積)` | ✅ |
| CHIDIST | カイ二乗分布（旧版） | `=CHIDIST(x, 自由度)` | ✅ |
| CHIINV | カイ二乗分布の逆関数（旧版） | `=CHIINV(確率, 自由度)` | ✅ |
| CHITEST | カイ二乗検定（旧版） | `=CHITEST(実測値範囲, 期待値範囲)` | ✅ |
| CONFIDENCE | 信頼区間（旧版） | `=CONFIDENCE(α, 標準偏差, 標本数)` | ✅ |
| CRITBINOM | 二項分布の臨界値（旧版） | `=CRITBINOM(試行回数, 成功率, α)` | ✅ |
| EXPONDIST | 指数分布（旧版） | `=EXPONDIST(x, λ, 累積)` | ✅ |
| FDIST | F分布（旧版） | `=FDIST(x, 自由度1, 自由度2)` | ✅ |
| FINV | F分布の逆関数（旧版） | `=FINV(確率, 自由度1, 自由度2)` | ✅ |
| FTEST | F検定（旧版） | `=FTEST(配列1, 配列2)` | ✅ |
| GAMMADIST | ガンマ分布（旧版） | `=GAMMADIST(x, α, β, 累積)` | ✅ |
| GAMMAINV | ガンマ分布の逆関数（旧版） | `=GAMMAINV(確率, α, β)` | ✅ |
| HYPGEOMDIST | 超幾何分布（旧版） | `=HYPGEOMDIST(標本成功数, 標本数, 母集団成功数, 母集団数)` | ✅ |
| LOGINV | 対数正規分布の逆関数（旧版） | `=LOGINV(確率, 平均, 標準偏差)` | ✅ |
| LOGNORMDIST | 対数正規分布（旧版） | `=LOGNORMDIST(x, 平均, 標準偏差)` | ✅ |
| MODE | 最頻値（旧版） | `=MODE(数値1, [数値2], ...)` | ✅ |
| NEGBINOMDIST | 負の二項分布（旧版） | `=NEGBINOMDIST(失敗数, 成功数, 成功率)` | ✅ |
| NORMDIST | 正規分布（旧版） | `=NORMDIST(x, 平均, 標準偏差, 累積)` | ✅ |
| NORMINV | 正規分布の逆関数（旧版） | `=NORMINV(確率, 平均, 標準偏差)` | ✅ |
| NORMSDIST | 標準正規分布（旧版） | `=NORMSDIST(z)` | ✅ |
| NORMSINV | 標準正規分布の逆関数（旧版） | `=NORMSINV(確率)` | ✅ |
| PERCENTILE | パーセンタイル（旧版） | `=PERCENTILE(配列, k)` | ✅ |
| PERCENTRANK | パーセント順位（旧版） | `=PERCENTRANK(配列, x, [有効桁数])` | ✅ |
| POISSON | ポアソン分布（旧版） | `=POISSON(x, 平均, 累積)` | ✅ |
| QUARTILE | 四分位数（旧版） | `=QUARTILE(配列, 戻り値)` | ✅ |
| RANK | 順位（旧版） | `=RANK(数値, 参照, [順序])` | ✅ |
| STDEV | 標準偏差（旧版） | `=STDEV(数値1, [数値2], ...)` | ✅ |
| STDEVP | 標準偏差・母集団（旧版） | `=STDEVP(数値1, [数値2], ...)` | ✅ |
| TDIST | t分布（旧版） | `=TDIST(x, 自由度, 尾部)` | ✅ |
| TINV | t分布の逆関数（旧版） | `=TINV(確率, 自由度)` | ✅ |
| TTEST | t検定（旧版） | `=TTEST(配列1, 配列2, 尾部, 検定の種類)` | ✅ |
| VAR | 分散（旧版） | `=VAR(数値1, [数値2], ...)` | ✅ |
| VARP | 分散・母集団（旧版） | `=VARP(数値1, [数値2], ...)` | ✅ |
| WEIBULL | ワイブル分布（旧版） | `=WEIBULL(x, α, β, 累積)` | ✅ |
| ZTEST | z検定（旧版） | `=ZTEST(配列, x, [σ])` | ✅ |
...
このライブラリの日付・時刻関数は、タイムゾーンとロケールに対応しています。

### 基本設定

```typescript
import { setGlobalDateConfig, TIMEZONES } from './src/utils/formulas/dateUtils';
...
| 地域 | タイムゾーン定数 | 説明 |
|------|------------------|------|
| 日本 | `TIMEZONES.JAPAN` | Asia/Tokyo |
| アメリカ東部 | `TIMEZONES.US_EASTERN` | America/New_York |
| アメリカ中部 | `TIMEZONES.US_CENTRAL` | America/Chicago |
| アメリカ山岳部 | `TIMEZONES.US_MOUNTAIN` | America/Denver |
| アメリカ西部 | `TIMEZONES.US_PACIFIC` | America/Los_Angeles |
| アメリカ・アラスカ | `TIMEZONES.US_ALASKA` | America/Anchorage |
| アメリカ・ハワイ | `TIMEZONES.US_HAWAII` | Pacific/Honolulu |
| イギリス | `TIMEZONES.UK` | Europe/London |
| フランス | `TIMEZONES.FRANCE` | Europe/Paris |
| ドイツ | `TIMEZONES.GERMANY` | Europe/Berlin |
| オーストラリア | `TIMEZONES.AUSTRALIA_SYDNEY` | Australia/Sydney |
| 中国 | `TIMEZONES.CHINA` | Asia/Shanghai |
| 韓国 | `TIMEZONES.KOREA` | Asia/Seoul |
| UTC | `TIMEZONES.UTC` | UTC |

### 関数別ローカライズ対応状況

| 関数 | タイムゾーン対応 | ロケール対応 | 備考 |
|------|:----------------:|:------------:|------|
| NOW | ✅ | ✅ | 現在日時を指定タイムゾーンで取得 |
| TODAY | ✅ | ✅ | 今日の日付を指定タイムゾーンで取得 |
| WEEKDAY | ✅ | - | タイムゾーンを考慮した曜日計算 |
| WEEKNUM | ✅ | - | タイムゾーンを考慮した週番号計算 |
| ISOWEEKNUM | ✅ | - | ISO週番号計算 |
| その他日付関数 | ✅ | - | タイムゾーン対応済み |
...
```typescript
import { getGlobalDateConfig } from './src/utils/formulas/dateUtils';

// 現在の設定を取得
const config = getGlobalDateConfig();
console.log(config.timezone); // 'Asia/Tokyo'
console.log(config.locale);   // 'ja-JP'
```

### カスタムフォーマット

```typescript
import { formatDate } from './src/utils/formulas/dateUtils';

const date = new Date();

// 日本語フォーマット
const jpFormat = formatDate(date, 'YYYY年MM月DD日', {
  locale: 'ja-JP',
  timezone: 'Asia/Tokyo'
});
